...
class Anualreport extends CI_Controller {
	...
	public function index() {
		$data['breadcrumb'] = "Anual Report";
		$data['results'] = $this->anualreport_model->index();

		$this->load->view('admin/templates/header', $data);
		$this->load->view('admin/anualreport/index');
	...
	public function add() {
		...
		$this->load->view('admin/templates/header', $data);
		$this->load->view('admin/anualreport/add');
		$this->load->view('admin/templates/footer');
	}
	...
	public function save() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->master_model->do_upload('anualreport', time()); // Call file upload function

			if ($this->master_model->upload->do_upload('userfile')) {
				$file_name = $this->upload->data('file_name');
				$this->master_model->img_resize('anualreport', $file_name, 2552, 3017); // Resize image after upload
			} else {
				$file_name = '';
			}
			...
			if ($this->anualreport_model->save($data)) {
	...
	public function edit($id) {
		$data['breadcrumb'] = "Anual Report";
		$data['edit'] = $this->anualreport_model->get_by_id($id);

		$this->load->view('admin/templates/header', $data);
		$this->load->view('admin/anualreport/edit');
		$this->load->view('admin/templates/footer');
	}
	...
	public function update() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->master_model->upload->do_upload('userfile')) {
				$file_name = $this->upload->data('file_name');
				$this->master_model->img_resize('anualreport', $file_name, 2552, 3017); // Resize image after upload				
				$this->master_model->delete_file('anualreport', $this->input->post('old_file'));
			} else {
			...
			if ($this->anualreport_model->update($data, $id)) {
				$this->form_validation->resetForm();
				$this->session->set_tempdata("msg", "<span class='success'>" . updated_success() . "</span>", 5);
			} else {
				$this->session->set_tempdata("msg", "<span class='error'>" . exception() . "</span>", 5);
			}
			redirect('anualreport');
		}
	}

	public function delete($id, $file_name=NULL) {
		if ($this->anualreport_model->delete($id) == FALSE) {
			$this->session->set_tempdata("msg", "<span class='error'>" . exception() . "</span>", 5);
		} else {
			$this->master_model->delete_file('anualreport', $file_name);
